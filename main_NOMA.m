...
load('H_d_all');
  Power=zeros(length(error_t),1);
   Rate=zeros(L,length(error_t ),10);
...
for loop =1 : num_loop
    outerflag=1;      
    H=H_d_all(1:N,1:L,loop)/sqrt(noise_maxpower_original);
    noise_maxpower=1;

    for l=1:L
        H_error(l)=error*norm(H(:,l),'fro');
    end
%%  For different SNR  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf('  loop |  num_J  |  SNR  |  i  |  trans_SNR | relay_SNR \n');
    ...
    F_ini=randn(N,L)*sqrt(trans_maxpower/(N*L));
    F(:,:,1)=full(F_ini);
  
    num_iterative = 15000;
    for n  = 1 : num_iterative
       %%%%%  Optimize F  %%%%%
        [F_1,power_sub,s,innerflag] = Generate_beamforming_F(N,  L, H, H_error,...
